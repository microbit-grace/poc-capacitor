...
interface HexData {
    name: string;
    hex?: string; // Content for local files
    path?: string; // Path for canned files
}
...
const HomeScreen: React.FC = () => {
    ...
    const handleHexSelectionChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
        const selectedValue = event.target.value;
        if (selectedValue === BROWSE_VALUE) {
            setSelectedHex(null); // Reset selection
            fileInputRef.current?.click();
        } else {
            const hexFile = cannedHexFiles.find(file => file.path === selectedValue);
            if (hexFile) {
    ...
    const handleFlashButtonClick = async () => {
        ...
        if (selectedHex.hex) {
            // It's a local file, hex content is already loaded
            ...
            try {
                ...
                if (!response.ok) {
                    throw new Error(`Failed to fetch ${selectedHex.name}: ${response.statusText}`);
                }
                const hex = await response.text();
                startFlashing({ name: selectedHex.name, hex });
            } catch (error) {
                console.error("Error flashing canned hex file:", error);
                alert(`Failed to flash ${selectedHex.name}. Check console for details.`);
            }
        }
    };
    ...
    return (
        <div style={{ padding: '20px', textAlign: 'center' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', maxWidth: '300px', margin: '20px auto' }}>
                <button
                ...
                <select
                    ...
                    value={selectedHex?.path || BROWSE_VALUE}
                    style={{
                        padding: '10px 20px',
                ...
                >
                    <option value={BROWSE_VALUE}>Browse for file</option>
