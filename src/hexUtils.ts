...
export type HexArrayBuffer = Uint8Array;

export const createHexFileFromUniversal = (
  inputHex: HexArrayBuffer,
...
export class SizedHexData {
  data;
  size;
  constructor(data: HexArrayBuffer, size: number) {
    this.data = data;
    this.size = size;
  }
}
...
class irmHexUtils {
  ...
  resultHex: HexArrayBuffer | null = null;
  resultData: HexArrayBuffer | null = null;
  ...
  scanForDataHexAndBlockInternal = (
    datahex: HexArrayBuffer | null,
    hexBlock: number,
    universalhex: HexArrayBuffer,
  ...
  //
  // return false on failure
  scanForDataHex = (hex: HexArrayBuffer) => {
  ...
  applicationHexToData = (hex: HexArrayBuffer, hexBlock: number) => {
    this.scanInit();
    const { min, next } = hexBlockToAppRegion(hexBlock);
    this.scanAddrMin = min;
    this.scanAddrNext = next;
...
function setCheck(hex: HexArrayBuffer, hexIdx: number): boolean {
...
function lineCheck(hex: HexArrayBuffer, hexIdx: number): number {
  const count = hextobyte(hex, hexIdx + 1);
