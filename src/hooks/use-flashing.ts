...
import { FlashProgressStage, FlashProgressCallback } from "@microbit/microbit-connection";
...
export const useFlashing = () => {
    ...
    const updateStep: FlashProgressCallback = useCallback((progressStage: FlashProgressStage, progress?: number) => {
        const message = {
    ...
    const startFlashing = useCallback((download: { name: string; hex: string }) => {
        setOpen(true);
        setHex(download);
    }, []);
